// Generated by cpp11: do not edit by hand
// clang-format off

#include "readpdf_types.h"
#include "cpp11/declarations.hpp"
#include <R_ext/Visibility.h>

// readpdf.cpp
int tesseract_major_version();
extern "C" SEXP _readpdf_tesseract_major_version() {
  BEGIN_CPP11
    return cpp11::as_sexp(tesseract_major_version());
  END_CPP11
}
// readpdf.cpp
list tesseract_config();
extern "C" SEXP _readpdf_tesseract_config() {
  BEGIN_CPP11
    return cpp11::as_sexp(tesseract_config());
  END_CPP11
}
// readpdf.cpp
TessPtr tesseract_engine_internal(strings datapath, strings language, strings confpaths, strings opt_names, strings opt_values);
extern "C" SEXP _readpdf_tesseract_engine_internal(SEXP datapath, SEXP language, SEXP confpaths, SEXP opt_names, SEXP opt_values) {
  BEGIN_CPP11
    return cpp11::as_sexp(tesseract_engine_internal(cpp11::as_cpp<cpp11::decay_t<strings>>(datapath), cpp11::as_cpp<cpp11::decay_t<strings>>(language), cpp11::as_cpp<cpp11::decay_t<strings>>(confpaths), cpp11::as_cpp<cpp11::decay_t<strings>>(opt_names), cpp11::as_cpp<cpp11::decay_t<strings>>(opt_values)));
  END_CPP11
}
// readpdf.cpp
TessPtr tesseract_engine_set_variable(TessPtr ptr, const char * name, const char * value);
extern "C" SEXP _readpdf_tesseract_engine_set_variable(SEXP ptr, SEXP name, SEXP value) {
  BEGIN_CPP11
    return cpp11::as_sexp(tesseract_engine_set_variable(cpp11::as_cpp<cpp11::decay_t<TessPtr>>(ptr), cpp11::as_cpp<cpp11::decay_t<const char *>>(name), cpp11::as_cpp<cpp11::decay_t<const char *>>(value)));
  END_CPP11
}
// readpdf.cpp
logicals validate_params(strings params);
extern "C" SEXP _readpdf_validate_params(SEXP params) {
  BEGIN_CPP11
    return cpp11::as_sexp(validate_params(cpp11::as_cpp<cpp11::decay_t<strings>>(params)));
  END_CPP11
}
// readpdf.cpp
list engine_info_internal(TessPtr ptr);
extern "C" SEXP _readpdf_engine_info_internal(SEXP ptr) {
  BEGIN_CPP11
    return cpp11::as_sexp(engine_info_internal(cpp11::as_cpp<cpp11::decay_t<TessPtr>>(ptr)));
  END_CPP11
}
// readpdf.cpp
strings print_params(std::string filename);
extern "C" SEXP _readpdf_print_params(SEXP filename) {
  BEGIN_CPP11
    return cpp11::as_sexp(print_params(cpp11::as_cpp<cpp11::decay_t<std::string>>(filename)));
  END_CPP11
}
// readpdf.cpp
strings get_param_values(TessPtr api, strings params);
extern "C" SEXP _readpdf_get_param_values(SEXP api, SEXP params) {
  BEGIN_CPP11
    return cpp11::as_sexp(get_param_values(cpp11::as_cpp<cpp11::decay_t<TessPtr>>(api), cpp11::as_cpp<cpp11::decay_t<strings>>(params)));
  END_CPP11
}
// readpdf.cpp
strings ocr_file(std::string file, TessPtr ptr, bool HOCR);
extern "C" SEXP _readpdf_ocr_file(SEXP file, SEXP ptr, SEXP HOCR) {
  BEGIN_CPP11
    return cpp11::as_sexp(ocr_file(cpp11::as_cpp<cpp11::decay_t<std::string>>(file), cpp11::as_cpp<cpp11::decay_t<TessPtr>>(ptr), cpp11::as_cpp<cpp11::decay_t<bool>>(HOCR)));
  END_CPP11
}
// readpdf.cpp
int n_pages(const std::string & file_path, const std::string & opw, const std::string & upw);
extern "C" SEXP _readpdf_n_pages(SEXP file_path, SEXP opw, SEXP upw) {
  BEGIN_CPP11
    return cpp11::as_sexp(n_pages(cpp11::as_cpp<cpp11::decay_t<const std::string &>>(file_path), cpp11::as_cpp<cpp11::decay_t<const std::string &>>(opw), cpp11::as_cpp<cpp11::decay_t<const std::string &>>(upw)));
  END_CPP11
}
// readpdf.cpp
list get_poppler_config();
extern "C" SEXP _readpdf_get_poppler_config() {
  BEGIN_CPP11
    return cpp11::as_sexp(get_poppler_config());
  END_CPP11
}
// readpdf.cpp
std::vector<std::string> poppler_convert(const std::string & file_path, const std::string & format, const std::vector<int> & pages, const std::vector<std::string> & names, const double & dpi, const std::string & opw, const std::string & upw, const bool & antialiasing, const bool & text_antialiasing);
extern "C" SEXP _readpdf_poppler_convert(SEXP file_path, SEXP format, SEXP pages, SEXP names, SEXP dpi, SEXP opw, SEXP upw, SEXP antialiasing, SEXP text_antialiasing) {
  BEGIN_CPP11
    return cpp11::as_sexp(poppler_convert(cpp11::as_cpp<cpp11::decay_t<const std::string &>>(file_path), cpp11::as_cpp<cpp11::decay_t<const std::string &>>(format), cpp11::as_cpp<cpp11::decay_t<const std::vector<int> &>>(pages), cpp11::as_cpp<cpp11::decay_t<const std::vector<std::string> &>>(names), cpp11::as_cpp<cpp11::decay_t<const double &>>(dpi), cpp11::as_cpp<cpp11::decay_t<const std::string &>>(opw), cpp11::as_cpp<cpp11::decay_t<const std::string &>>(upw), cpp11::as_cpp<cpp11::decay_t<const bool &>>(antialiasing), cpp11::as_cpp<cpp11::decay_t<const bool &>>(text_antialiasing)));
  END_CPP11
}

extern "C" {
static const R_CallMethodDef CallEntries[] = {
    {"_readpdf_engine_info_internal",          (DL_FUNC) &_readpdf_engine_info_internal,          1},
    {"_readpdf_get_param_values",              (DL_FUNC) &_readpdf_get_param_values,              2},
    {"_readpdf_get_poppler_config",            (DL_FUNC) &_readpdf_get_poppler_config,            0},
    {"_readpdf_n_pages",                       (DL_FUNC) &_readpdf_n_pages,                       3},
    {"_readpdf_ocr_file",                      (DL_FUNC) &_readpdf_ocr_file,                      3},
    {"_readpdf_poppler_convert",               (DL_FUNC) &_readpdf_poppler_convert,               9},
    {"_readpdf_print_params",                  (DL_FUNC) &_readpdf_print_params,                  1},
    {"_readpdf_tesseract_config",              (DL_FUNC) &_readpdf_tesseract_config,              0},
    {"_readpdf_tesseract_engine_internal",     (DL_FUNC) &_readpdf_tesseract_engine_internal,     5},
    {"_readpdf_tesseract_engine_set_variable", (DL_FUNC) &_readpdf_tesseract_engine_set_variable, 3},
    {"_readpdf_tesseract_major_version",       (DL_FUNC) &_readpdf_tesseract_major_version,       0},
    {"_readpdf_validate_params",               (DL_FUNC) &_readpdf_validate_params,               1},
    {NULL, NULL, 0}
};
}

extern "C" attribute_visible void R_init_readpdf(DllInfo* dll){
  R_registerRoutines(dll, NULL, CallEntries, NULL, NULL);
  R_useDynamicSymbols(dll, FALSE);
  R_forceSymbols(dll, TRUE);
}
